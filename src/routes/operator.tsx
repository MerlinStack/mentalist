...
function OperatorConsole() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0F1E] text-slate-100 flex flex-col font-sans antialiased selection:bg-indigo-500/30">
      ...
      <header className="h-16 border-b border-[#2D3A5C]/40 bg-[#1A2035]/80 backdrop-blur-md flex items-center justify-between px-6 shadow-lg z-20">
        <div className="flex items-center gap-3">
          <div className="h-8 w-8 rounded-lg bg-gradient-to-tr from-blue-600 to-indigo-500 shadow-md shadow-blue-500/30 flex items-center justify-center font-bold text-sm text-white">
            D
          </div>
          <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-4 py-1.5 text-xs font-semibold bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg shadow-md shadow-blue-500/20 hover:brightness-110 disabled:opacity-30 disabled:pointer-events-none transition"
          >
            Push Live →
          </button>
        </div>
      ...
      <main className="flex-1 grid grid-cols-12 gap-6 p-6 h-[calc(100vh-4rem)] overflow-hidden">
        ...
        <section className="col-span-3 flex flex-col gap-4 h-full">
          ...
          <div className="p-4 rounded-xl bg-[#1A2035]/40 backdrop-blur-md border border-[#2D3A5C]/40 shadow-inner">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Audio Waveform</span>
              <span className="text-[10px] font-mono text-slate-500">{sessionTime}</span>
            </div>
          ...
          <div className="flex flex-col h-[200px] p-4 rounded-xl bg-[#1A2035]/40 backdrop-blur-md border border-[#2D3A5C]/40 shadow-inner">
            <div className="flex items-center justify-between mb-3 border-b border-white/5 pb-2">
              <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Live Audio Capture</h3>
          ...
          <div className="p-4 rounded-xl bg-[#1A2035]/20 border border-[#2D3A5C]/20 flex-1 text-[11px] font-mono space-y-2.5 text-slate-400 overflow-y-auto">
            <h4 className="font-semibold uppercase text-[9px] tracking-wider text-slate-500 border-b border-white/5 pb-1 mb-1">Detection Log</h4>
              ...
              detectionHistory.map((item, i) => (
                <div key={i} className="p-2 rounded bg-black/30 border-l-2 border-emerald-500/50 space-y-0.5">
                  ...
                  <div className="flex items-center gap-2 text-[10px] text-slate-500">
                    <span className="px-1 rounded bg-emerald-500/10 text-emerald-400 text-[9px] font-medium">Regex</span>
        ...
        <section className="col-span-9 grid grid-cols-2 gap-6 h-full">
          ...
          <div className="relative rounded-2xl border border-[#2D3A5C] bg-gradient-to-b from-[#1A2035]/30 to-transparent p-8 flex flex-col items-center justify-center min-h-[450px] shadow-2xl transition-all duration-300 hover:border-blue-500/30">
            <span className="absolute top-4 left-4 text-[10px] font-bold tracking-widest uppercase text-blue-400 px-2.5 py-1 bg-blue-500/10 rounded-md border border-blue-500/20 shadow-sm font-mono">
              Preview Stage Board
            </span>
            ...
            {detectedVerse ? (
              <div className="text-center space-y-6 max-w-md animate-fadeIn">
                ...
                <p className="text-2xl font-serif leading-relaxed text-white font-medium drop-shadow-md">
          ...
          <div className="relative rounded-2xl border-2 border-red-500/30 bg-black/40 p-8 flex flex-col items-center justify-center min-h-[450px] shadow-2xl transition-all duration-300">
            <span className="absolute top-4 left-4 text-[10px] font-bold tracking-widest uppercase text-rose-500 px-2.5 py-1 bg-rose-500/10 rounded-md border border-rose-500/20 flex items-center gap-2 shadow-sm font-mono">
            ...
            {currentVerse && (
              <button
                ...
                className="absolute top-4 right-4 text-[10px] text-slate-400 hover:text-white px-2 py-1 rounded bg-slate-800 border border-slate-700 transition"
              >
                Clear Screen
              </button>
            )}
            ...
            {currentVerse ? (
              <div className="text-center space-y-4 max-w-md animate-scaleUp">
                <p className="text-2xl font-serif leading-relaxed text-white font-semibold">
                  {currentVerse.text}
                </p>
                <span className="text-xs font-mono text-slate-400 tracking-wider block uppercase font-bold text-blue-400">
      ...
      <footer className="h-10 border-t border-[#2D3A5C]/30 bg-[#1A2035]/60 backdrop-blur-sm flex items-center justify-between px-6 shrink-0">
        ...
        <div className="flex items-center gap-3">
          <span className="text-[10px] text-slate-600">Translation</span>
          <button
            onClick={() => setTranslation((i) => (i + 1) % TRANSLATIONS.length)}
            className="px-2 py-0.5 rounded text-[11px] font-mono text-slate-400 bg-slate-800 border border-slate-700"
          >
